...
class EphFrame(wx.Frame):
    ...
    def initUI(self):
        ...
        self.addTextCtrl(5,0, self.longitude, wx.StaticText(self, -1, 'Longitude (deg:min:sec):'))
        self.addTextCtrl(6,0, self.elevation, wx.StaticText(self, -1, 'Observatory Elevation: '))
        self.addTextCtrl(7,0, self.temp, wx.StaticText(self, -1, 'Temperature (Celcius): '))
        self.addTextCtrl(8,0, self.v_limit, wx.StaticText(self,-1, 'v_limit: '))
        self.addTextCtrl(9,0, self.depth_limit, wx.StaticText(self,-1,'Depth Limit: '))
        self.addTextCtrl(10,0, self.twilightType, wx.StaticText(self,-1, 'Twilight Type: '))
        self.addRadioBox(4,3, self.html_out)
        self.addRadioBox(6,3, self.text_out)
        self.addRadioBox(8,3, self.calc_eclipses)
    ...
    def loadValues(self, obsPath):
        ...
        for line in obsPath:
            if line.split(':',1) > 1 and line[0] != '#':
                ...
                #elif line[0] == 'elevation':   str(line[1].split('#')[0].strip())
                ##elif line[0] == 'temperature':   str(line[1].split('#')[0].strip())
                #elif line[0] == 'min_horizon':    str(line[1].split('#')[0].strip())
                #elif line[0] == 'start_date': str(line[1].split('#')[0].strip())
                #elif line[0] == 'end_date': str(line[1].split('#')[0].strip())
                #elif line[0] == 'v_limit': str(line[1].split('#')[0].strip())
                #elif line[0] == 'depth_limit': str(line[1].split('#')[0].strip())
                #elif line[0] == 'calc_eclipses': bool(line[1].split('#')[0].strip())
                #elif line[0] == 'html_out': self.outputPath = inline[1].split('#')[0].strip()
               # elif line[0] == 'text_out': self.outputPath = inline[1].split('#')[0].strip()
                #elif line[0] == 'twilight': self.outputPath = inline[1].split('#')[0].strip()
        obsPath.close()
