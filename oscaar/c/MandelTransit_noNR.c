 ...
 2013-12-08 11:44:07 Kyle Pearson: Editted to remove use of NR routines
 ...
 # create so for mandel transits
g++ -c -fopenmp -fPIC MandelTransits_noNR.c -o mandel.o 
g++ -g -shared -fopenmp -Wl,-soname,lib_mandel.so -o lib_mandel.so mandel.o
...
extern "C" void occultquad(double *t, double p, double ap, double P, double i, double gamma1, double gamma2, double e, double longPericenter, double t0, double n, double *F);
...
void occultquad(double *t, double p, double ap, double P, double i, double gamma1, double gamma2, double e, double longPericenter, double t0, double n, double *F)
{
    ...
    new_t = (double *) malloc(sizeof(double)*Npoints); //assert(var != NULL);    
